...
void Test::Test_Unit() {
  std::cout << "Begin to test matrix class now \n";
  //Below is list out multiple test cases for needed operators of the matrix template class
  Test_Case(test_case_number, ".operator=", test_matrix_ = 2, test_matrix_(2,2) == 2 );
  //Now, let's test the getting opeators at (row,colum) or at(index) options
  Test_Case(test_case_number, ".operator(row,column)=", NULL, test_matrix_(2,2) == 2 );
  Test_Case(test_case_number, ".operator(index)="     , NULL, test_matrix_(9-1)   == 2 );
  //How about when we passing the invalid positions to it?
  Test_Case(test_case_number, ".operator(invalid_index)", NULL, test_matrix_(9) != 1 );
  Test_Case(test_case_number, ".operator(negative_index)", NULL, test_matrix_(-1) != 1 );
  //How about the assigning value to matrix elements at valid position
  Test_Case(test_case_number, ".operator&(row,column)", test_matrix_(1,1) = 8, test_matrix_(1,1) == 8 );
  Test_Case(test_case_number, ".operator&(index)", test_matrix_(4) = 12, test_matrix_(4) == 12 );
...
void Test::Test_Integrate() {
  std::cout << "Begin to test tictactoe class now \n";
  ...
  Test_Case(test_case_number, ".exist VALID crossed row?", NULL , test_game_.Is_Cross_Row() == false );
  Test_Case(test_case_number, ".exist VALID crossed column?", NULL , test_game_.Is_Cross_Column() == false );
  Test_Case(test_case_number, ".exist VALID crossed diagonal?", NULL , test_game_.Is_Cross_Diagonal() == false );
